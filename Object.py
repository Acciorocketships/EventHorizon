...
class Object:
	...
	def __init__(self, uid=None, objects={}, sim=DefaultSim()):
		self.objects = objects
		self.sim = sim
		self.uid = uid
		self.accel = torch.zeros(3)
		self.obj_time = self.sim.time
	...
	@property
	def mass(self):
		return p.getDynamicsInfo(bodyUniqueId=self.uid, linkIndex=-1, physicsClientId=self.sim.id)[0]
	...
	def set_force(self, force=[0,0,0], pos=None, body=False):
		if pos is None:
			if body:
				pos = [0,0,0]
			else:
				pos = self.get_pos()
		force = totensor(force)
		pos = totensor(pos)
		flag = p.LINK_FRAME if body else p.WORLD_FRAME
		if self.sim.time != self.obj_time:
			self.accel = torch.zeros(3)
			self.obj_time = self.sim.time
		self.accel += force / self.mass
		p.applyExternalForce(objectUniqueId=self.uid, linkIndex=-1, forceObj=force.tolist(), posObj=pos.tolist(), flags=flag, physicsClientId=self.sim.id)


	def set_torque(self, torque=[0,0,0], body=False):
		torque = totensor(torque)
		flag = p.WORLD_FRAME if body else p.LINK_FRAME
		p.applyExternalTorque(objectUniqueId=self.uid, linkIndex=-1, torqueObj=torque.tolist(), flags=flag, physicsClientId=self.sim.id)
