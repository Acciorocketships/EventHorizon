...
class Network:
	...
	def send(self, var, name='var', host=None, port=None):
		...
		currport = port
		if host is None:
			currhost = self.otherhost
		if port is None:
			currport = self.otherport
		if self.otherhost is None:
			self.otherhost = currhost
		if self.otherport is None:
			self.otherport = currport
		if currhost == self.otherhost and currport == self.otherport:
			if not self.initsender:
				self.s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
				self.s.connect((currhost, currport))
				self.initsender = True
			connection = self.s
		else:
			connection = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
			connection.connect((currhost, currport))
		# Set up data to send
